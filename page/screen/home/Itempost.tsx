...
import {
    Alert,
  SafeAreaView, ScrollView, Text, View, Image, TouchableOpacity, StyleSheet, FlatList ,Button, Dimensions, ActivityIndicator,
} from 'react-native';
...
import { ChoiceType, OptionType ,Reserve_SelectBoxType } from '../../../components/types/componentType'
...
const Itempost = ({route}:Props) => {
    ...
    const [selectReserve, setSelReserve] = React.useState<OptionType>({
        label : t("상태변경"), value : '예약상태변경' ,  sel_id: 0
    })
    ...
    const slideImage = [
        items.data[0].pt_image1,
        items.data[0].pt_image2,
        items.data[0].pt_image3,
        items.data[0].pt_image4,
        items.data[0].pt_image5,
        items.data[0].pt_image6,
        items.data[0].pt_image7,
        items.data[0].pt_image8,
        items.data[0].pt_image9,
        items.data[0].pt_image10,
    ]
    const [filterslideImage,setfilterslideImage] = React.useState<any>([
    ])
    
    const FilterAction = ()=>{
        if(slideImage[0] == null){
            return
        } else{
            console.log(slideImage)
            let filter1 = slideImage.filter(e=>e !== null)
            let filter = filter1.filter(e=>e !== "")
            setfilterslideImage(filter)
        }
        setIsLoading(false)
    }
    ...
    <SafeAreaView style={[style.default_background , {flex:1}]}>
        ...
        <LoadingIndicator/>
        :
        <SwiperSlide gofullscreen={gofullscreen} imageheight={410} 
        SlideImage={isloading? slideImage:filterslideImage}
        />
        }
        <View style={{marginVertical:24,marginHorizontal:20}}>
            ...
            <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:16}}>
                <View style={{ flexDirection:'row' , marginTop:10}}>
                    ...
                    <Text style={[style.text_me ,{backgroundColor:colors.BLUE_COLOR_1, 
                        borderRadius:5, color:colors.WHITE_COLOR , marginRight:7,
                        paddingVertical:3,paddingHorizontal:6,fontSize:12
                        }]}>
                    ...
                    <Text style={[style.text_me,{backgroundColor:colors.GRAY_COLOR_5, 
                    borderRadius:5, fontSize:12, marginRight:7, color:colors.WHITE_COLOR, 
                ...
                <Text style={[style.text_me ,{backgroundColor:colors.GRAY_COLOR_1,borderRadius:5
                    ,color:colors.GRAY_COLOR_2,paddingVertical:3,paddingHorizontal:10, fontSize:12,}]}>
            ...
            <View style={{borderBottomWidth:1, borderColor:colors.GRAY_COLOR_3, marginBottom:20}}>
                <Text style={[style.text_me, {fontSize:15, color:colors.BLACK_COLOR_2 ,marginBottom:4}]}>
            ...
            <View style={{alignItems:'flex-end',marginVertical:25,}}>
                ...
                <Image style={{
                    width: 15 , height: 15, marginRight:4, marginLeft:8}} source={require('../../../assets/img/ico_view.png')}/>
                <Text style={[style.text_re, {fontSize:12, color:colors.GRAY_COLOR_2}]}>
                {items.data[0].pt_hit}
                </Text>
                <Image style={{
                    width: 15 , height: 15, marginRight:4, marginLeft:8}} source={require('../../../assets/img/ico_comment.png')}/>
                <Text style={[style.text_re, {fontSize:12, color:colors.GRAY_COLOR_2}]}>
                {items.data[0].pt_chat}
                </Text>
                <Image style={{
                    width: 15 , height: 15, marginRight:4, marginLeft:8}} source={require('../../../assets/img/ico_book2.png')}/>
                <Text style={[style.text_re, {fontSize:12, color:colors.GRAY_COLOR_2}]}>
                {items.data[0].pt_wish}
                </Text>
                </View>
            </View>
            <View style={{flexDirection:'row',paddingHorizontal:20, paddingVertical:16, backgroundColor:colors.GRAY_COLOR_1,borderRadius:5}}>
                <View style={{flex:1,marginRight:20}}>
                <TouchableOpacity onPress={()=>{setProfileToggle(true)}}
                style={{flexDirection:'row'}}>
                    <Image 
                        style={{width:44, height:44, marginRight:20}}
                        source={require('../../../assets/img/img_profile.png')} resizeMode='cover' borderRadius={100}
                    />
                    <View style={{}}>
                        ...
                        {t("판매상품수")} {items.selling_count}  · {t("거래완료 횟수")} {items.pt_end_cnt == undefined? 0:items.pt_end_cnt}
                        </Text>
                    </View>
                </TouchableOpacity>
                </View>

                <TouchableOpacity onPress={()=>ReportPost(items.data[0].mt_seller_idx)}>
        ...
        />
        <View style={{
            borderTopWidth:1, borderColor:colors.GRAY_COLOR_3,
            flexDirection:'row', justifyContent:'space-between', height:60, paddingHorizontal:20, paddingVertical:12}}>
            <View style={{justifyContent:'center'}}>
                <Text style={[style.default_font_black,{fontSize:18,}]}> 
                ￦ {NumberComma(items.data[0].pt_selling_price)}</Text>
            </View>
            <View style={{flexDirection:'row',}}>
                <TouchableOpacity onPress={()=>heartOn(items.data[0].pt_idx)}
                style={{marginRight:6, padding:10, backgroundColor:colors.GRAY_COLOR_1, justifyContent:'center', borderRadius:5 }}>
                    {items.wp_idx ?
                    <Image 
                    style={{width:25, height:25, }}
                ...
                <TouchableOpacity 
                onPress={()=>Reserve_choice({
                    id:items.data[0].pt_idx,
                    image:items.data[0].pt_image1,
                    title:items.data[0].pt_title
                })}
                style={{backgroundColor:colors.GREEN_COLOR_2, alignItems:'center',justifyContent:'center',
